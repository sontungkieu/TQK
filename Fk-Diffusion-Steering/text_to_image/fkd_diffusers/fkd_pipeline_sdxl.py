...
# #region agent log
_debug_log(
    location="fkd_pipeline_sdxl.py:import",
...
try:
    ...
    _debug_log(
        location="fkd_pipeline_sdxl.py:import",
        message="Attempt import .fkd_class and .rewards",
...
class FKDStableDiffusionXL(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        callback_on_step_end_tensor_inputs_checked = callback_on_step_end_tensor_inputs
        if callback_on_step_end_tensor_inputs and "x0_preds" in callback_on_step_end_tensor_inputs:
            callback_on_step_end_tensor_inputs_checked = [
                val
                for val in callback_on_step_end_tensor_inputs
                if val != "x0_preds"
            ]
        ...
        if fkd_args is not None:
            custom_reward_fn = fkd_args.get("custom_reward_fn")
        ...
        def postprocess_and_apply_reward_fn(x):
            ...
            if custom_reward_fn is not None:
                rewards = custom_reward_fn(imagesx, prompt)
            else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # FK Steering Change
                if custom_reward_fn is not None:
                    set_timestep = getattr(custom_reward_fn, "set_timestep", None)
                    if callable(set_timestep):
                        latest_step_idx = (
                            fkd_args.get("dift_latest_step_idx") if fkd_args else None
                        )
                        latest_t_value = (
                            fkd_args.get("dift_latest_timestep_value") if fkd_args else None
                        )
                        if (
                            latest_step_idx is not None
                            and latest_t_value is not None
                            and i >= latest_step_idx
                        ):
                            set_timestep(latest_t_value)
                        else:
                            set_timestep(t)
                ...
                if fkd_args is not None and fkd_args.get("use_smc") and fkd_args.get("log_reward_every_step"):
                    ...
                    _debug_log(
                        location="fkd_pipeline_sdxl.py:__call__",
                        message="Per-step reward stats",
                        data={
                            "sampling_idx": int(i),
        ...
        if fkd_args is not None and fkd_args.get("use_smc") and "fkd" in locals():
            ...
            flat_rewards = [
                val for step in fkd.trajectory_log.get("rewards", []) for val in step
            ]
            _debug_log(
                location="fkd_pipeline_sdxl.py:__call__",
                message="Attach trajectory log",
                data={
                    "timesteps_len": len(fkd.trajectory_log.get("timesteps", [])),
                    "rewards_min": float(min(flat_rewards)) if flat_rewards else None,
                    "rewards_max": float(max(flat_rewards)) if flat_rewards else None,
                    "first_rewards": fkd.trajectory_log.get("rewards", [None])[0],
                },
                hypothesis_id="H9",
            )
        return output
