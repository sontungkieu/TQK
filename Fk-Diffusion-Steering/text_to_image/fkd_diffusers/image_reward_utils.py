...
class IRSMC(nn.Module):
    ...
    def _prepare_tensor_images(
        ...
        *,
        ...
        if emulate_pil_uint8_roundtrip:
            images = (images * 255.0).round().clamp(0.0, 255.0) / 255.0
    ...
    def score_batched_tensor(
        ...
        *,
        ...
        if not isinstance(image_tensors, torch.Tensor):
            raise TypeError("image_tensors must be a torch.Tensor")

        profile = {"tensor_preprocess_s": 0.0, "ir_model_forward_s": 0.0}
        ...
        rewards = self.mlp(txt_features)
        rewards = (rewards - self.mean) / self.std
        profile["ir_model_forward_s"] = time.perf_counter() - model_start

        scores = rewards.view(txt_features.shape[0]).detach().cpu().numpy().tolist()
        if return_profile:
