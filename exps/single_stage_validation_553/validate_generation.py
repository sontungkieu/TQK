...
def main() -> None:
    ...
    rows = []
    for path in sorted((EXP / "metadata").glob("gpu*/*_*.json")):
        ...
        assert row["fresh_seed_base"] == 20260919
        trace = row["trace"]
        if method == "psp":
    ...
    assert len(rows) == args.expected_prompts * 2, (len(rows), args.expected_prompts * 2)
    for prompt_id in range(args.expected_prompts):
        pair = [row for row in rows if int(row["prompt_id"]) == prompt_id]
        assert {row["method"] for row in pair} == {"psp", "ours"}
