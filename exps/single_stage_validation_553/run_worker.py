...
def explicit_pool(pipe, prompt_id: int, pool_size: int) -> tuple[torch.Tensor, list[int], list[str]]:
    seeds = [FRESH_SEED_BASE + prompt_id * 32 + candidate_id for candidate_id in range(pool_size)]
...
def complete(prompt_id: int, method: str, worker_index: int, expected_evals: int) -> bool:
...
def main() -> None:
    ...
    atomic_json(EXP / "metadata" / f"gpu{args.worker_index}" / "hardware.json", {
    ...
    for row in owned:
        ...
        pending = [
            method for method, spec in methods.items()
            if not (args.resume and complete(prompt_id, method, args.worker_index, logical_evals(spec)))
        ]
        if not pending:
            completed += 1
            continue
        pool, seeds, hashes = explicit_pool(pipe, prompt_id, pool_size)
        for method in pending:
            ...
            atomic_json(EXP / "metadata" / f"gpu{args.worker_index}" / f"{prompt_id:05d}_{method}.json", result)
            print(json.dumps({key: result[key] for key in (
                "prompt_id", "method", "logical_unet_evals", "batched_verifier_calls",
                "verifier_candidate_scores", "elapsed_s", "peak_vram_gib", "final_image_reward"
            )}), flush=True)
        completed += 1
