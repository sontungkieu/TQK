...
def main() -> None:
    ...
    for start in range(0, len(missing), batch_size):
        ...
        text = tokenizer([prompts[prompt_id] for _method, prompt_id in chunk]).to(device)
        ...
        print(json.dumps({"hps_complete": len(existing), "hps_total": 1106}), flush=True)
    assert len(existing) == 1106
    print(output)
