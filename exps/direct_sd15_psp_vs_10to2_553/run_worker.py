...
def load_prompts() -> list[dict[str, Any]]:
    path = EXP / "prompts_geneval_all_553.jsonl"
    rows = [json.loads(line) for line in path.read_text().splitlines() if line.strip()]
    ids = [row["prompt_id"] for row in rows]
    assert len(rows) == 553 and len(set(ids)) == 553
    return rows
...
def main() -> None:
    ...
    assignment_payload = json.loads((EXP / "worker_assignments_553.json").read_text())
    assignments = {int(k): int(v) for k, v in assignment_payload["assignments"].items()}
    assert len(assignments) == 553 and set(assignments.values()) == {0, 1}
    owned = [row for row in prompts if assignments[int(row["prompt_id"])] == args.worker_index]
    pipe = build_pipeline()
