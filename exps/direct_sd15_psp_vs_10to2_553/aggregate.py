...
def read_prompts() -> pd.DataFrame:
    ...
    assert len(frame) == 553 and frame.prompt_id.nunique() == 553
...
def read_generation() -> pd.DataFrame:
    ...
    frame = pd.DataFrame(rows)
    assert len(frame) == 1106, len(frame)
...
def main() -> None:
    ...
    ids = json.loads((EXP / "prompt_ids_553.json").read_text())["prompt_ids"]
    ...
    for tag in sorted(prompts.tag.unique()):
        ...
        category_n = int((prompts.tag == tag).sum())
        category_lines.append(
            f"| {tag} | {category_n} | {rows.psp:.4f} | {rows.early10to2:.4f} | "
    ...
    hw_lines.append(
        f"| Combined | 553 | {wall/3600:.3f} h | {combined_util_text} | {shards.max_vram_gib.max():.2f} GiB |"
    )

    report = f"""# Direct SD1.5 PSP vs 10→2 on all 553 GenEval prompts
...
Paired inference over 553 prompts: {interpretation(stats)} The GenEval paired outcomes are **{win_tie_loss['ours_win']} ours wins / {win_tie_loss['tie']} ties / {win_tie_loss['psp_win']} PSP wins**.
...
{interpretation(stats)} This conclusion is restricted to SD1.5, the full 553-prompt GenEval set, this single repetition, and the two pre-specified schedules. No further schedule was selected.
"""
