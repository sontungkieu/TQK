...
def main(args):
    full_results = []
    eval_jobs = []
    samples_dir_name = args.samples_dir
    for subfolder in os.listdir(args.imagedir):
        ...
        # Collect jobs first (same filtering logic as before), so tqdm has a known total.
        for imagename in os.listdir(os.path.join(folderpath, samples_dir_name)):
            imagepath = os.path.join(folderpath, samples_dir_name, imagename)
            if not os.path.isfile(imagepath) or not re.match(r"\d+\.png", imagename):
                continue
            eval_jobs.append((imagepath, metadata))

    # Run the exact same evaluation as before, only wrapped with a global progress bar.
    for imagepath, metadata in tqdm(eval_jobs, total=len(eval_jobs), desc="Evaluating images"):
